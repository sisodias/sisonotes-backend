import { Container } from '@blocksuite/sisonotes/global/di';
import type {
  ...
  ImageBlockModel,
  TableBlockModel,
} from '@blocksuite/sisonotes/model';
import { SisoNotesSchemas } from '@blocksuite/sisonotes/schemas';
import { MarkdownAdapter } from '@blocksuite/sisonotes/shared/adapters';
import type { SisoNotesTextAttributes } from '@blocksuite/sisonotes/shared/types';
import {
  createYProxy,
  type DeltaInsert,
  type DraftModel,
  ...
  type YBlock,
} from '@blocksuite/sisonotes/store';
...
blocksuiteSchema.register([...SisoNotesSchemas]);
...
const bookmarkFlavours = new Set([
  'sisonotes:bookmark',
  'sisonotes:embed-youtube',
  'sisonotes:embed-figma',
  'sisonotes:embed-github',
  'sisonotes:embed-loom',
]);

const collectInlineReferences = (
  deltas: DeltaInsert<SisoNotesTextAttributes>[]
...
const getTextDeltasFromCellValue = (
  value: unknown
): DeltaInsert<SisoNotesTextAttributes>[] | null => {
  if (!value) {
    return null;
  }

  if (value instanceof YText) {
    return value.toDelta() as DeltaInsert<SisoNotesTextAttributes>[];
  }

  if (typeof value === 'object' && value !== null) {
    const maybeText = value as { yText?: unknown };
    if (maybeText.yText instanceof YText) {
      return maybeText.yText.toDelta() as DeltaInsert<SisoNotesTextAttributes>[];
    }
  }
    ...
    if (marker) {
      if (delta instanceof YArray) {
        return delta
          ...
          | DeltaInsert<SisoNotesTextAttributes>[]
          | null;
      }
      if (Array.isArray(delta)) {
        return delta as DeltaInsert<SisoNotesTextAttributes>[];
      }
    }
  }
  ...
  if (
    ...
    if (delta instanceof YArray) {
      return delta.toArray() as DeltaInsert<SisoNotesTextAttributes>[];
    }
    if (Array.isArray(delta)) {
      return delta as DeltaInsert<SisoNotesTextAttributes>[];
    }
...
function generateMarkdownPreviewBuilder(
  ...
  function getListDepth(block: BlockDocumentInfo) {
    ...
    do {
      ...
      if (!currentBlock || currentBlock.flavour !== 'sisonotes:list') {
        break;
      }
      parentBlockCount++;
    } while (currentBlock);
  ...
  const generateDatabaseMarkdownPreview = (block: BlockDocumentInfo) => {
    const isDatabaseBlock = (block: BlockDocumentInfo) => {
      return block.flavour === 'sisonotes:database';
    };
  ...
  const generateImageMarkdownPreview = (block: BlockDocumentInfo) => {
    const isImageModel = (
      ...
      return model?.flavour === 'sisonotes:image';
    };
  ...
  const generateEmbedMarkdownPreview = (block: BlockDocumentInfo) => {
    const isEmbedModel = (
      ...
      return (
        model?.flavour === 'sisonotes:embed-linked-doc' ||
        model?.flavour === 'sisonotes:embed-synced-doc'
      );
    };
  ...
  const generateAttachmentMarkdownPreview = (block: BlockDocumentInfo) => {
    const isAttachmentModel = (
      ...
      return model?.flavour === 'sisonotes:attachment';
    };
  ...
  const generateTableMarkdownPreview = (block: BlockDocumentInfo) => {
    const isTableModel = (
      ...
      return model?.flavour === 'sisonotes:table';
    };
  ...
  const generateMarkdownPreview = async (block: BlockDocumentInfo) => {
    ...
    } else if (flavour === 'sisonotes:attachment') {
      markdown = generateAttachmentMarkdownPreview(block);
    } else if (flavour === 'sisonotes:image') {
      markdown = generateImageMarkdownPreview(block);
    } else if (flavour === 'sisonotes:surface' || flavour === 'sisonotes:page') {
      // skip
    } else if (flavour === 'sisonotes:latex') {
...
> {
  ...
  for (const block of blocks.values()) {
    ...
    if (flavour === 'sisonotes:page' && blockId) {
      rootBlockId = blockId;
    }
  }
  ...
  while (queue.length) {
    ...
    const noteBlock = nearestByFlavour(blockId, 'sisonotes:note');
    ...
    if (flavour === 'sisonotes:page') {
      ...
      const deltas: DeltaInsert<SisoNotesTextAttributes>[] = text.toDelta();
      const refs = collectInlineReferences(deltas);

      const databaseName =
        flavour === 'sisonotes:paragraph' && parentFlavour === 'sisonotes:database' // if block is a database row
    ...
    } else if (flavour === 'sisonotes:surface') {
      const texts = [];
  ...
  for (const block of blockDocuments) {
    if (block.ref?.length) {
      ...
      // should only generate the markdown preview belong to the same sisonotes:note
      const noteBlock = nearestByFlavour(block.blockId, 'sisonotes:note');

      const sameNoteBlocks = noteBlock
        ? blockDocuments.filter(
            candidate =>
              nearestByFlavour(candidate.blockId, 'sisonotes:note') === noteBlock
          )
        : [];
