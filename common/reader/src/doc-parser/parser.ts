import type { ColumnDataType } from '@blocksuite/sisonotes/model';
...
export function parseBlock(
  ...
  try {
    switch (flavour) {
      ...
      case 'sisonotes:embed-linked-doc':
      case 'sisonotes:embed-synced-doc': {
      ...
      case 'sisonotes:surface':
      case 'sisonotes:page':
      case 'sisonotes:note':
      case 'sisonotes:frame': {
        result.content = '';
        break;
      }
      case 'sisonotes:database': {
        ...
        const optionToTagHtml = (option: any) => {
          return `<span data-sisonotes-option data-value="${option.id}" data-option-color="${option.color}">${option.value}</span>`;
        };
  ...
  if (result.content && aiEditable && blockLevel === 2) {
    ...
    if (flavour === 'sisonotes:database' || placeholder) {
...
export const parsePageDoc = (ctx: ParserContext): ParsedDoc => {
  ...
  const maybePageBlock = Object.entries(yBlocks.toJSON()).findLast(
    ([_, b]) => b['sys:flavour'] === 'sisonotes:page'
  );
  ...
  if (!maybePageBlock) {
    ...
    rootBlock.children = rootBlock.children.filter(
      (block): block is BaseParsedBlock => block.flavour === 'sisonotes:note'
    );
