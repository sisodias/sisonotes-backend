...
SELECT
  p."workspace_id",
  p."user_id",
  sisonotes_permission_legacy_workspace_role(p."type"),
  'active',
...
WHERE p."status" = 'Accepted'::"WorkspaceMemberStatus"
  AND sisonotes_permission_legacy_workspace_role(p."type") IS NOT NULL
...
SELECT
  ...
  p."user_id",
  p."inviter_id",
  CASE sisonotes_permission_legacy_workspace_role(p."type")
    WHEN 'admin' THEN 'admin'
    ELSE 'member'
  END,
  sisonotes_permission_workspace_invitation_state(p."status"),
  CASE p."source"::text WHEN 'Link' THEN 'link' ELSE 'email' END,
  p."created_at",
  p."updated_at"
FROM "workspace_user_permissions" p
WHERE p."status" <> 'Accepted'::"WorkspaceMemberStatus"
  AND sisonotes_permission_legacy_workspace_role(p."type") IS NOT NULL
  AND sisonotes_permission_workspace_invitation_state(p."status") IS NOT NULL
ON CONFLICT DO NOTHING;
...
SELECT
  ...
  sisonotes_permission_legacy_default_doc_role(p."defaultRole"),
  p."published_at"
...
SELECT
  ...
  'user',
  p."user_id",
  sisonotes_permission_legacy_doc_role(p."type"),
  p."created_at"
FROM "workspace_page_user_permissions" p
WHERE sisonotes_permission_legacy_doc_role(p."type") IS NOT NULL
ON CONFLICT DO NOTHING;
...
BEGIN
  ...
  JOIN "workspace_members" m
   ...
   AND m."state" = 'active'
  WHERE p."status" = 'Accepted'::"WorkspaceMemberStatus"
    AND sisonotes_permission_legacy_workspace_role(p."type") IS DISTINCT FROM m."role";
  ...
  WHERE p."status" <> 'Accepted'::"WorkspaceMemberStatus"
    AND (
      sisonotes_permission_workspace_invitation_state(p."status") IS DISTINCT FROM i."status"
      OR CASE sisonotes_permission_legacy_workspace_role(p."type")
        WHEN 'admin' THEN 'admin'
        ELSE 'member'
  ...
  WHERE p."visibility" IS DISTINCT FROM CASE WHEN d."public" THEN 'public' ELSE 'private' END
    OR p."member_default_role" IS DISTINCT FROM sisonotes_permission_legacy_default_doc_role(d."defaultRole");
  ...
  WHERE sisonotes_permission_legacy_doc_role(d."type") IS DISTINCT FROM g."role";
  ...
  FROM "workspace_page_user_permissions" d
  WHERE sisonotes_permission_legacy_doc_role(d."type") IS NOT NULL
    AND NOT EXISTS (
