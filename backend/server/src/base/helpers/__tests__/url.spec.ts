...
test('can factor base url correctly without specified external url', t => {
  t.is(t.context.url.baseUrl, 'https://app.sisonotes.local');
});
...
test('can create link', t => {
  t.is(t.context.url.link('/path'), 'https://app.sisonotes.local/path');
  t.is(
    t.context.url.link('/path', { a: 1, b: 2 }),
    'https://app.sisonotes.local/path?a=1&b=2'
  );
  t.is(
    t.context.url.link('/path', { a: 1, b: '/path' }),
    'https://app.sisonotes.local/path?a=1&b=%2Fpath'
  );
});
...
test('addSimpleQuery should not double encode', t => {
  t.is(
    t.context.url.addSimpleQuery(
      'https://app.sisonotes.local/path',
      'redirect_uri',
      '/path'
    ),
    'https://app.sisonotes.local/path?redirect_uri=%2Fpath'
  );
});
...
test('addSimpleQuery should allow unescaped value when escape=false', t => {
  t.is(
    ...
    'https://app.sisonotes.local/path?session_id={CHECKOUT_SESSION_ID}'
  );
});
...
test('can validate callbackUrl allowlist', t => {
  ...
  t.true(
    t.context.url.isAllowedCallbackUrl('https://app.sisonotes.local/magic-link')
  );
  t.false(
...
test('can create safe link', t => {
  t.is(t.context.url.safeLink('/path'), 'https://app.sisonotes.local/path');
...
test('can safe redirect', t => {
  ...
  [
    'https://app.sisonotes.local',
    'https://app.sisonotes.local/path',
    'https://app.sisonotes.local/path?query=1',
  ].forEach(allow);
  ['https://other.domain.com', 'a://invalid.uri'].forEach(deny);
});

test('can get request origin', t => {
  t.is(t.context.url.requestOrigin, 'https://app.sisonotes.local');
});

test('can get request base url', t => {
  t.is(t.context.url.requestBaseUrl, 'https://app.sisonotes.local');
});
...
test('can get request base url with multiple hosts', t => {
  ...
  const url = new URLHelper(
    {
      server: {
        externalUrl: '',
        host: 'app.sisonotes.local1',
        hosts: ['app.sisonotes.local1', 'app.sisonotes.local2'],
        port: 3010,
        https: true,
        path: '',
      },
  ...
  t.is(url.requestOrigin, 'https://app.sisonotes.local1');
  t.is(url.requestBaseUrl, 'https://app.sisonotes.local1');

  // set cls
  cls.set(CLS_REQUEST_HOST, 'app.sisonotes.local2');
  t.is(url.requestOrigin, 'https://app.sisonotes.local2');
  t.is(url.requestBaseUrl, 'https://app.sisonotes.local2');
});

test('should allow websocket secure origin by normalizing wss to https', t => {
  const allowedOrigins = buildCorsAllowedOrigins({
    allowedOrigins: ['https://app.sisonotes.pro'],
  } as any);

  t.true(isCorsOriginAllowed('wss://app.sisonotes.pro', allowedOrigins));
});
