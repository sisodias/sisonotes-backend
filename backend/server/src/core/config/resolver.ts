...
export class ServerConfigResolver {
  ...
  async availableUpgrade(): Promise<ReleaseVersionType | null> {
    ...
    const url = `https://sisonotes.pro/api/worker/releases?channel=${channel}`;
    ...
    try {
      ...
      if (!response.ok) {
        this.logger.error(
          'failed to fetch sisonotes releases',
          await response.text()
        );
        return null;
      }
      ...
      this.logger.error('failed to fetch sisonotes releases', e);
      return null;
    }
  }
}
