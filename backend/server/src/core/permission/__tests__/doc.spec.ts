...
test.beforeEach(async () => {
  ...
  user = await models.user.create({ email: `${randomUUID()}@sisonotes.pro` });
  ...
  },
  {
    ...
    setup: async () => {
      const u2 = await models.user.create({
        email: `${randomUUID()}@sisonotes.pro`,
      });
...
test('should assert action', async t => {
  ...
  const u2 = await models.user.create({ email: `${randomUUID()}@sisonotes.pro` });
