...
function initTestStaticFiles(staticPath: string) {
  const files = {
    ...
    'assets-manifest.json': JSON.stringify({
      js: ['main.a.js'],
      css: [],
      publicPath: 'https://app.sisonotes.pro/',
      gitHash: '',
      description: '',
    }),
...
test.before(async t => {
  const staticPath = new Package('@siso/server').join('static').value;
...
test('should render correct html', async t => {
  ...
  t.true(
    res.text.includes(
      `<script src="https://app.sisonotes.pro/main.a.js" crossorigin></script>`
    )
  );
});
...
test.skip('should render correct mobile html', async t => {
  ...
  t.true(
    res.text.includes(
      `<script src="https://app.sisonotes.pro/main.c.js" crossorigin></script>`
    )
  );
});
