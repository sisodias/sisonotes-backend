...
import { installLicenseMutation, SubscriptionVariant } from '@siso/graphql';
...
e2e('should not install expired license', async t => {
  await t.throwsAsync(
    ...
    {
      message:
        'Invalid license to activate. License file has expired. Please contact with SisoNotes support to fetch a latest one.',
    }
  );
});
