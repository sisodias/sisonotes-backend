...
import { transformToForm } from '@siso/graphql';
...
export class TestingApp extends ApplyType<INestApplication>() {
  ...
  request(
    ...
    if (this.csrfCookie) {
      req.set('x-sisonotes-csrf-token', this.csrfCookie);
    }
  ...
  // TODO(@forehalo): directly make proxy for graphql queries defined in `@siso/graphql`
  ...
  private randomEmail() {
    return `test-${randomUUID()}@sisonotes.pro`;
  }
