import serverNativeModule from '@siso/server-native';
...
const mockDispatch = () =>
  (async function* (): AsyncIterableIterator<LlmToolLoopStreamEvent> {
    ...
    yield { type: 'citation', index: 1, url: 'https://sisonotes.pro' };
  ...
  const cases = [
    {
      ...
      verify: (text: string) => {
        ...
        t.true(
          text.includes('[^1]: {"type":"url","url":"https%3A%2F%2Fsisonotes.pro"}')
        );
      },
    },
    {
