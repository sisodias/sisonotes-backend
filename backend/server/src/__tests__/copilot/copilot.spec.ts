...
import { ProjectRoot } from '@siso-tools/utils/path';
...
test.beforeEach(async t => {
  ...
  prompt.reset();
  const user = await auth.signUp(`test-${randomUUID()}@sisonotes.pro`, '123456');
  userId = user.id;
...
test('should reject context file uploads after workspace write access is revoked', async t => {
  ...
  const owner = await auth.signUp(`test-${randomUUID()}@sisonotes.pro`, '123456');
  const member = await auth.signUp(`test-${randomUUID()}@sisonotes.pro`, '123456');
...
test('should be able to render listed prompt', async t => {
  ...
  const params = {
    links: ['https://sisonotes.pro', 'https://github.com/sisoagency/sisonotes'],
  };
  ...
  t.is(
    prompt.finish(testPrompt!, params).pop()?.content,
    'links:\n- https://sisonotes.pro\n- https://github.com/sisoagency/sisonotes\n',
    'should render the prompt'
  );
});
...
test('should be able to fork chat session', async t => {
  ...
  const newUser = await auth.signUp('darksky.1@sisonotes.pro', '123456');
...
test('should merge latest user turn content and attachments into prompt', async t => {
  ...
  for (const testCase of [
    ...
    },
    {
      ...
      message: { attachments: ['https://sisonotes.pro/example.jpg'] as string[] },
      project: (messages: { attachments?: unknown }[]) =>
        messages.map(({ attachments }) => attachments),
      expected: [undefined, ['https://sisonotes.pro/example.jpg']],
    },
    {
      title: 'empty message',
      message: {},
