  ...
  const app = await createTestingApp({
    imports: [
      ConfigModule.override({
        ...
        server: {
          hosts: ['localhost', 'test.sisonotes.dev'],
          https: true,
        },
      }),
      AppModule,
...
test.beforeEach(async t => {
  ...
  t.context.u1 = await t.context.auth.signUp('u1@sisonotes.pro', '1');
});
...
test('should be able to redirect to oauth provider with multiple hosts', async t => {
  ...
  t.is(
    redirect.searchParams.get('redirect_uri'),
    'https://test.sisonotes.dev/oauth/callback'
  );
  t.is(redirect.searchParams.get('response_type'), 'code');
  t.is(redirect.searchParams.get('prompt'), 'select_account');
  t.truthy(redirect.searchParams.get('state'));
...
test('should be able to redirect to oauth provider with client_nonce', async t => {
  ...
  const res = await app
    .POST('/api/oauth/preflight')
    .send({ provider: 'Google', client: 'sisonotes', client_nonce: '1234567890' })
    .expect(HttpStatus.OK);
...
test('should record sign in client version from oauth preflight state', async t => {
  ...
  const preflightRes = await app
    .POST('/api/oauth/preflight')
    .set('x-sisonotes-version', '0.25.3')
    .send({ provider: 'Google', client: 'web', client_nonce: 'test-nonce' })
    .expect(HttpStatus.OK);
  ...
  Sinon.stub(provider, 'getUser').resolves({
    id: '1',
    email: 'oauth-version@sisonotes.pro',
    avatarUrl: 'avatar',
  });
...
function mockOidcProvider(
  ...
  Sinon.stub(
    ...
    userinfo_endpoint: 'https://oidc.sisonotes.dev/userinfo',
  }));
  Sinon.stub(
...
function createOidcRegistrationHarness(config?: {
  ...
  const sisonotesConfig = {
    server: {
      externalUrl: 'https://sisonotes.example',
      host: 'localhost',
      path: '',
      https: true,
      hosts: [],
    ...
    oauth: {
      providers: {
        oidc: {
          ...
          issuer: config?.issuer ?? 'https://issuer.sisonotes.dev',
...
test('should be able to sign up with oauth', async t => {
  ...
  const clientNonce = mockOAuthProvider(app, 'u2@sisonotes.pro');
  ...
  t.is(sessionUser!.email, 'u2@sisonotes.pro');
  ...
  const user = await db.user.findFirst({
    ...
    where: {
      email: 'u2@sisonotes.pro',
    },
  });

  t.truthy(user);
  t.is(user!.email, 'u2@sisonotes.pro');
...
test('should be able to sign up with oauth and client_nonce', async t => {
  ...
  const userEmail = `${clientNonce}@sisonotes.pro`;
...
test('should be able to fullfil user with oauth sign in', async t => {
  ...
  const u3 = await app.createUser('u3@sisonotes.pro');
...
test('github oauth should resolve private email from emails api', async t => {
  ...
  const email = 'github-private@sisonotes.pro';
  ...
  fetchJson.onSecondCall().resolves([
    { email: 'unverified@sisonotes.pro', primary: true, verified: false },
...
test('github oauth should reject responses without a verified email', async t => {
  ...
  fetchJson
    .onSecondCall()
    .resolves([
      { email: 'private@sisonotes.pro', primary: true, verified: false },
    ]);
...
test('oidc should accept email from id token when userinfo email is missing', async t => {
  ...
  mockOidcProvider(provider, {
    idTokenClaims: {
      sub: 'oidc-user',
      email: 'oidc-id-token@sisonotes.pro',
      name: 'OIDC User',
    },
    userinfo: {
      sub: 'oidc-user',
  ...
  t.is(user.email, 'oidc-id-token@sisonotes.pro');
...
test('oidc should resolve custom email claim from userinfo', async t => {
  ...
  mockOidcProvider(provider, {
    ...
    userinfo: {
      sub: 'oidc-user',
      mail: 'oidc-userinfo@sisonotes.pro',
  ...
  t.is(user.id, 'oidc-user');
  t.is(user.email, 'oidc-userinfo@sisonotes.pro');
...
test('oidc should resolve custom email claim from id token', async t => {
  ...
  mockOidcProvider(provider, {
    ...
    idTokenClaims: {
      sub: 'oidc-user',
      mail: 'oidc-custom-id-token@sisonotes.pro',
      mail_verified: 'true',
    },
    userinfo: {
      sub: 'oidc-user',
  ...
  t.is(user.id, 'oidc-user');
  t.is(user.email, 'oidc-custom-id-token@sisonotes.pro');
});
...
test('oidc should not fall back to default email claim when custom claim is configured', async t => {
  ...
  mockOidcProvider(provider, {
    ...
    idTokenClaims: {
      sub: 'oidc-user',
      email: 'fallback@sisonotes.pro',
    },
    userinfo: {
      sub: 'oidc-user',
      email: 'userinfo-fallback@sisonotes.pro',
    },
  });
...
test('oidc discovery should remove oauth feature on failure and restore it after backoff retry succeeds', async t => {
  const issuer = 'https://auth.internal/application/o/sisonotes/';
