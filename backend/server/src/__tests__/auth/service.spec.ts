...
test('should be able to sign in by password', async t => {
  ...
  const signedInUser = await auth.signIn('u1@sisonotes.pro', '1');

  t.is(signedInUser.email, 'u1@sisonotes.pro');
});

test('should throw if user not found', async t => {
  const { auth } = t.context;

  await t.throwsAsync(() => auth.signIn('u2@sisonotes.pro', '1'), {
    message: 'Wrong user email or password: u2@sisonotes.pro',
  });
});
...
test('should throw if password not match', async t => {
  ...
  await t.throwsAsync(() => auth.signIn('u1@sisonotes.pro', '2'), {
    message: 'Wrong user email or password: u1@sisonotes.pro',
  });
});
...
test('should be able to change password', async t => {
  ...
  let signedInU1 = await auth.signIn('u1@sisonotes.pro', '1');
  t.is(signedInU1.email, u1.email);

  await auth.changePassword(u1.id, 'hello world sisonotes');

  await t.throwsAsync(
    () => auth.signIn('u1@sisonotes.pro', '1' /* old password */),
    {
      message: 'Wrong user email or password: u1@sisonotes.pro',
    }
  );

  signedInU1 = await auth.signIn('u1@sisonotes.pro', 'hello world sisonotes');
  t.is(signedInU1.email, u1.email);
});
...
test('rolls back email change when session revocation fails', async t => {
  ...
  await t.throwsAsync(() =>
    auth.changeEmailAndRevokeSessions(u1.id, 'replacement@sisonotes.pro')
  );
  revoke.restore();
...
test('should be able to change email', async t => {
  ...
  await auth.changeEmail(u1.id, 'u2@sisonotes.pro');

  await t.throwsAsync(() => auth.signIn('u1@sisonotes.pro' /* old email */, '1'), {
    message: 'Wrong user email or password: u1@sisonotes.pro',
  });

  signedInU1 = await auth.signIn('u2@sisonotes.pro', '1');
  t.is(signedInU1.email, 'u2@sisonotes.pro');
});
...
test('should be able to sign in different user in a same session', async t => {
  ...
  const u2 = await auth.signUp('u2@sisonotes.pro', '1');
