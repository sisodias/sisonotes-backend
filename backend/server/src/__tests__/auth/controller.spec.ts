...
test('should be able to sign in with credential', async t => {
  ...
  const u1 = await app.createUser('u1@sisonotes.pro');
...
test('should accept one Hashcash proof and reject its replay', async t => {
  ...
  const user = await app.createUser('hashcash-login@sisonotes.pro');
  config.override({
    captcha: {
      enabled: true,
      config: {
  ...
  const challenge = await app
    ...
    .set('x-sisonotes-client-kind', 'native')
    .expect(200);
...
test('should validate Turnstile action and reject query credentials', async t => {
  const { app, config } = t.context;
  const user = await app.createUser('turnstile-login@sisonotes.pro');
  config.override({
    captcha: {
      enabled: true,
      config: {
...
test('should reject a Turnstile response for another action', async t => {
  const { app, config } = t.context;
  const user = await app.createUser('turnstile-action@sisonotes.pro');
  config.override({
    captcha: {
      enabled: true,
      config: {
...
test('should expose a retryable error when Turnstile is unavailable', async t => {
  const { app, config } = t.context;
  const user = await app.createUser('turnstile-unavailable@sisonotes.pro');
  config.override({
    captcha: {
      enabled: true,
      config: {
...
test('should issue exchange code only for native credential sign in', async t => {
  ...
  const u1 = await app.createUser('native@sisonotes.pro');
...
test('should rotate token pairs and manage auth sessions', async t => {
  const { app } = t.context;
  const user = await app.createUser('token-lifecycle@sisonotes.pro');
...
test('should revoke another device immediately and revoke all devices', async t => {
  const { app } = t.context;
  const user = await app.createUser('device-lifecycle@sisonotes.pro');
...
test('should require csrf and revoke cookie plus auth sessions', async t => {
  ...
  const user = await app.createUser('cookie-revoke-all@sisonotes.pro');
  ...
  await app
    .POST('/api/auth/sessions/revoke-all')
    .set('Cookie', cookies)
    .set('x-sisonotes-csrf-token', parsed[AuthService.csrfCookieName])
    .expect(201);
...
test('should idempotently revoke a auth session with its refresh token', async t => {
  const { app } = t.context;
  const user = await app.createUser('token-revoke@sisonotes.pro');
  const signIn = await app
    .POST('/api/auth/sign-in')
    .set('x-sisonotes-client-kind', 'native')
    .send({ email: user.email, password: user.password })
...
test('should roll back user-session issuance when auth-session issuance fails', async t => {
  const { app } = t.context;
  const user = await app.createUser('token-rollback@sisonotes.pro');
  const signIn = await app
    .POST('/api/auth/sign-in')
    .set('x-sisonotes-client-kind', 'native')
    .send({ email: user.email, password: user.password })
...
for (const userState of ['disabled', 'deleted'] as const) {
  test(`should reject exchange after the user is ${userState}`, async t => {
    const { app, db } = t.context;
    const user = await app.createUser(`token-${userState}@sisonotes.pro`);
    const signIn = await app
      .POST('/api/auth/sign-in')
      .set('x-sisonotes-client-kind', 'native')
      .send({ email: user.email, password: user.password })
...
test('should allow an authenticated device to revoke its session', async t => {
  const { app, db } = t.context;
  const user = await app.createUser('token-recent-auth@sisonotes.pro');
  const signIn = await app
    .POST('/api/auth/sign-in')
    .set('x-sisonotes-client-kind', 'native')
    .send({ email: user.email, password: user.password })
...
test('should not issue jwt for browser-origin credential sign in', async t => {
  ...
  const u1 = await app.createUser('browser@sisonotes.pro');

  const res = await app
    .POST('/api/auth/sign-in')
    .set('origin', 'https://app.sisonotes.pro')
    .set('x-sisonotes-client-kind', 'native')
    .send({ email: u1.email, password: u1.password })
    .expect(200);
...
test('should preserve Electron 0.26 cookie authentication', async t => {
  const { app, db } = t.context;
  const user = await app.createUser('electron-026@sisonotes.pro');

  const signIn = await app
    .POST('/api/auth/sign-in')
    .set('x-sisonotes-version', '0.26.7')
    .send({
      email: user.email,
...
test('should not fall back to a cookie on auth-session refresh', async t => {
  const user = await t.context.app.createUser('refresh-cookie@sisonotes.pro');
  await t.context.app
    .POST('/api/auth/sign-in')
    .send({ email: user.email, password: user.password })
    .expect(200);
...
test('should record sign in client version when header is provided', async t => {
  ...
  await app
    .POST('/api/auth/sign-in')
    .set('x-sisonotes-version', '0.25.1')
    .send({ email: u1.email, password: u1.password })
    .expect(200);
...
test('should return method-oriented preflight for unknown users', async t => {
  ...
  const res = await app
    .POST('/api/auth/preflight')
    .send({ email: 'unknown@sisonotes.pro' })
    .expect(201);
...
test('should return password unavailable for registered users without password', async t => {
  ...
  const u1 = await app.createUser('passwordless@sisonotes.pro', {
    password: null,
  });
...
test('should return methods unavailable for disabled users', async t => {
  ...
  const u1 = await app.createUser('disabled@sisonotes.pro', {
    disabled: true,
  });
...
test('should return magic link unavailable when domain verification rejects signup email', async t => {
  ...
  const res = await app
    .POST('/api/auth/preflight')
    .send({ email: 'unknown+alias@sisonotes.pro' })
    .expect(201);
...
test('should return bound auth methods for current account', async t => {
  ...
  await app.signupV1('bound-methods@sisonotes.pro');
...
test('should be able to sign up with email', async t => {
  ...
  const res = await app
    .POST('/api/auth/sign-in')
    .send({ email: 'u2@sisonotes.pro' })
    .expect(200);

  t.is(res.body.email, 'u2@sisonotes.pro');
  const signUpMail = app.mails.last('SignUp');

  t.is(signUpMail.to, 'u2@sisonotes.pro');
  ...
  const session = await currentUser(app);
  t.is(session?.email, 'u2@sisonotes.pro');
});
...
test('should ignore user_id query when signing out with jwt', async t => {
  ...
  const u1 = await app.createUser('u1@sisonotes.pro');
  const u2 = await app.createUser('u2@sisonotes.pro');
...
test('should be able to sign out when duplicated csrf cookies exist', async t => {
  ...
  await supertest(app.getHttpServer())
    .post('/api/auth/sign-out')
    .set('Cookie', cookieHeader)
    .set('x-sisonotes-csrf-token', csrf)
    .expect(200);
...
test('should reject sign out when csrf token mismatched', async t => {
  ...
  await app
    .POST('/api/auth/sign-out')
    .set('x-sisonotes-csrf-token', 'invalid')
    .expect(HttpStatus.FORBIDDEN);
...
test('should be able to correct user id cookie', async t => {
  ...
  const u1 = await app.signupV1('u1@sisonotes.pro');
...
test('should be able to sign out multiple accounts in one session', async t => {
  ...
  const u1 = await app.signupV1('u1@sisonotes.pro');
  const u2 = await app.signupV1('u2@sisonotes.pro');
...
test('should not be able to sign in if token is invalid', async t => {
  ...
  const res = await app
    .POST('/api/auth/magic-link')
    .send({ email: 'u1@sisonotes.pro', token: 'invalid' })
    .expect(400);
