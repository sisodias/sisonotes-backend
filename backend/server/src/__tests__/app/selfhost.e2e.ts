...
function initTestStaticFiles(staticPath: string) {
  const files = {
    'selfhost.html': `<!DOCTYPE html><html><body>SISO Notes</body><script src="main.a.js"/></html>`,
    'main.a.js': `const name = 'sisonotes'`,
    'admin/selfhost.html': `<!DOCTYPE html><html><body>SISO Notes Admin</body><script src="/admin/main.b.js"/></html>`,
    'admin/main.b.js': `const name = 'sisonotes-admin'`,
    'mobile/selfhost.html': `<!DOCTYPE html><html><body>SISO Notes mobile</body><script src="/mobile/main.c.js"/></html>`,
    'mobile/main.c.js': `const name = 'sisonotes-mobile'`,
  };
...
test('should always return static asset files', async t => {
  let res = await request(t.context.app.getHttpServer())
    .get('/main.a.js')
    .expect(200);
  t.is(res.text, "const name = 'sisonotes'");

  res = await request(t.context.app.getHttpServer())
    .get('/admin/main.b.js')
    .expect(200);
  t.is(res.text, "const name = 'sisonotes-admin'");

  res = await request(t.context.app.getHttpServer())
    .get('/main.c.js')
    .expect(200);
  t.is(res.text, "const name = 'sisonotes-mobile'");

  await t.context.db.user.create({
    data: {
      name: 'test',
      email: 'test@sisonotes.pro',
    },
  });
...
test('should allow visiting setup page if not initialized', async t => {
  ...
  t.true(res.text.includes('SISO Notes Admin'));
});
...
test.skip('should return web assets if visited by mobile', async t => {
  ...
  t.true(res.text.includes('SISO Notes mobile'));
});
