...
test('should create a new user', async t => {
  ...
  t.is(user.email, 'test@sisonotes.pro');

  const user2 = await t.context.user.getUserByEmail('test@sisonotes.pro');

  t.not(user2, null);
  t.is(user2!.email, 'test@sisonotes.pro');
});
...
test('should update an user', async t => {
  ...
  const user2 = await t.context.user.update(user.id, {
    email: 'test2@sisonotes.pro',
  });

  t.is(user2.email, 'test2@sisonotes.pro');
});
...
test('should return null for non existing user', async t => {
  const user = await t.context.user.getUserByEmail('test@sisonotes.pro');
...
test('should fulfill user', async t => {
  ...
  const user2 = await t.context.user.fulfill('test2@sisonotes.pro');
...
test('should delete user with pending invitation missing normalized email', async t => {
  const owner = await t.context.user.create({
    email: 'owner@sisonotes.pro',
  });
  const invitee = await t.context.user.create({
    email: 'invitee@sisonotes.pro',
    registered: false,
  });
...
test('should paginate users', async t => {
  ...
  await Promise.all(
    Array.from({ length: 100 }).map((_, i) =>
      t.context.user.create({
        ...
        email: `test-paginate-${i}@sisonotes.pro`,
  ...
  t.deepEqual(
    ...
    Array.from({ length: 10 }).map((_, i) => `test-paginate-${i}@sisonotes.pro`)
  );
});
