...
test('should read NAMESPACE', t => {
  t.deepEqual(
    ['dev', 'beta', 'production'].map(envVal => {
      process.env.SISO_NOTES_ENV = envVal;
      const env = new Env();
  ...
  t.throws(() => {
    process.env.SISO_NOTES_ENV = 'unknown';
    new Env();
  });
});
...
test('should tell selfhosted correctly', t => {
  ...
  process.env.DEPLOYMENT_TYPE = 'sisonotes';
  t.false(new Env().selfhosted);
});

test('should tell namespaces correctly', t => {
  process.env.SISO_NOTES_ENV = 'dev';
  ...
  process.env.SISO_NOTES_ENV = 'beta';
  t.deepEqual(new Env().namespaces, {
    canary: false,
    beta: true,
    production: false,
  ...
  process.env.SISO_NOTES_ENV = 'production';
  t.deepEqual(new Env().namespaces, {
    canary: false,
    beta: false,
    production: true,
