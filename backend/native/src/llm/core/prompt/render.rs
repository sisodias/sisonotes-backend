...
fn render_prompt_messages(
  ...
  render_context.retain(|key, _| !key.starts_with("sisonotes::"));
...
pub(super) fn create_prompt_builtins(params: &Map<String, Value>) -> Map<String, Value> {
  ...
  Map::from_iter([
    (
      "sisonotes::date".to_string(),
      Value::String(Local::now().format("%-m/%-d/%Y").to_string()),
    ),
    (
      "sisonotes::language".to_string(),
      Value::String(
        params
          .get("language")
          .and_then(Value::as_str)
    ...
    ("sisonotes::hasDocsRef".to_string(), Value::Bool(has_docs)),
    ("sisonotes::hasFilesRef".to_string(), Value::Bool(has_files)),
    ("sisonotes::hasSelected".to_string(), Value::Bool(has_selected)),
    ("sisonotes::hasCurrentDoc".to_string(), Value::Bool(has_current_doc)),
  ])
}
