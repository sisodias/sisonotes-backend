  ...
  fn should_render_prompt_with_normalized_params_and_attachments() {
    let response = llm_render_prompt(
      serde_json::from_value::<PromptRenderContract>(json!({
        ...
        "renderParams": {
          "attachments": ["https://sisonotes.pro/example.jpg"],
          "content": "hello world"
        }
      }))
      .unwrap(),
    ...
    let response = llm_render_prompt(
      serde_json::from_value::<PromptRenderContract>(json!({
        "messages": [
          {
            "role": "system",
            "content": "{{sisonotes::language}}|{{tags}}|{{obj}}|{{#links}}- {{.}}\n{{/links}}"
          }
        ],
        "templateParams": {},
        "renderParams": {
          "language": "French",
          "sisonotes::language": "ignored",
          "links": ["https://sisonotes.pro", "https://github.com/sisoagency/sisonotes"],
          "obj": { "hello": "world" },
          "tags": ["a", "b"]
        }
    ...
    assert_eq!(
      ...
      json!({
        "messages": [
          {
            "role": "system",
            "content": "French|a,b|[object Object]|- https://sisonotes.pro\n- https://github.com/sisoagency/sisonotes\n",
            "params": {
              "language": "French",
              "sisonotes::language": "ignored",
              "links": ["https://sisonotes.pro", "https://github.com/sisoagency/sisonotes"],
  ...
  fn should_render_session_prompt_by_merging_latest_user_content() {
    let response = llm_render_session_prompt(
      serde_json::from_value::<PromptSessionContract>(json!({
        ...
        "turns": [
          { "role": "user", "content": "hello", "attachments": ["https://sisonotes.pro/hello.png"] }
        ],
        "renderParams": {},
        "maxTokenSize": 1000
      }))
      ...
      json!({
        "messages": [
          { "role": "system", "content": "answer briefly", "params": { "content": "hello" } },
          {
            "role": "user",
            "content": "hello",
            "attachments": ["https://sisonotes.pro/hello.png"],
