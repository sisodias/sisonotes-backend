...
// - SISO Notes-native-owned: model-registry projection and transcript/action
//   product contracts.
...
fn schema_by_name(name: &str) -> Option<Value> {
  match name {
    ...
    >()),
    // SISO Notes-native-owned N-API projection over adapter model registry/matcher
    ...
    // SISO Notes-native-owned prompt facade over adapter prompt DTOs/catalog
    ...
    // SISO Notes-native-owned product transcript contracts
