...
impl Runtime {
  ...
  fn run(&mut self) -> Result<ActionRuntimeOutput> {
    ...
    let mut executor = SisoNotesActionStepExecutor::new(&self.control, attachments.clone());
...
struct SisoNotesActionStepExecutor<'a> {
  #[cfg(test)]
...
impl<'a> SisoNotesActionStepExecutor<'a> {
...
impl RecipeStepExecutor for SisoNotesActionStepExecutor<'_> {
  fn execute_step(
    &mut self,
    step: &RecipeStepExecution,
    input: Option<Value>,
