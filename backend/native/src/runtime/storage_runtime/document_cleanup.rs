...
fn root_contains(root: CurrentDoc, doc_id: &str) -> RuntimeResult<bool> {
  let projection = sisonotes_doc_loader::project_workspace_root(root.blob, true)
...
mod tests {
  ...
  async fn doc_blob_refs_projection_semantics_and_document_cleanup_mark_only_postgres() -> AnyResult<()> {
    ...
    let root = sisonotes_doc_loader::add_doc_to_root_doc(Vec::new(), "live-doc", None)?;
    let live_doc = sisonotes_doc_loader::build_full_doc("Live", "", "live-doc")?;
    let doc = sisonotes_doc_loader::build_full_doc("Missing", "![Asset](blob://candidate-blob)", doc_id)?;
    sqlx::query(
      r#"
    ...
    sqlx::query("UPDATE snapshots SET blob = $2 WHERE workspace_id = $1 AND guid = 'live-doc'")
      .bind(&workspace_id)
      .bind(sisonotes_doc_loader::build_full_doc("Live", "", "live-doc")?)
      .execute(&pool)
      .await?;
    ...
    sqlx::query("UPDATE snapshots SET blob = $2 WHERE workspace_id = $1 AND guid = $1")
      .bind(&workspace_id)
      .bind(sisonotes_doc_loader::add_doc_to_root_doc(Vec::new(), doc_id, None)?)
      .execute(&pool)
      .await?;
  ...
  async fn document_cleanup_execute_postgres_semantics() -> AnyResult<()> {
    ...
    let root = sisonotes_doc_loader::add_doc_to_root_doc(Vec::new(), live_doc_id, None)?;
    let live_doc = sisonotes_doc_loader::build_full_doc("Live", "", live_doc_id)?;
    let missing_doc = sisonotes_doc_loader::build_full_doc("Doc", "![Alt](blob://image-blob-key)", doc_id)?;
    sqlx::query(
      r#"
    INSERT INTO snapshots (workspace_id, guid, blob, updated_at)
    VALUES ($1, $1, $2, CURRENT_TIMESTAMP),
    ...
    .bind(&workspace_id)
    .bind(doc_id)
    .bind(sisonotes_doc_loader::add_doc_to_root_doc(
      Vec::new(),
      "update-block",
      None,
    )?)
    ...
    let retry_doc = sisonotes_doc_loader::build_full_doc("Retry", "", retry_doc_id)?;
    sqlx::query(
