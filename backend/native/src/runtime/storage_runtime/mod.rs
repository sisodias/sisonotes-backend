...
mod tests {
  ...
  fn workspace_live_set_merges_pending_updates_and_includes_trash() {
    ...
    let snapshot = sisonotes_doc_loader::add_doc_to_root_doc(Vec::new(), "live", None).unwrap();
    let pending = sisonotes_doc_loader::add_doc_to_root_doc(snapshot.clone(), "trash", None).unwrap();
