...
async fn verification_token_sql_state_machine_handles_keep_verify_and_cleanup() {
  ...
  let mismatch_token = runtime
    .create_verification_token(
      ...
      Some("user@sisonotes.test".to_string()),
      30_000,
    )
    .await
    .unwrap();
  assert!(
    runtime
      .verify_verification_token(
        ...
        Some("wrong@sisonotes.test".to_string()),
        None,
      )
      .await
      .unwrap()
  ...
  let keep_token = runtime
    .create_verification_token(
      TEST_VERIFICATION_TOKEN_TYPE,
      Some("keep@sisonotes.test".to_string()),
      30_000,
    )
    .await
    .unwrap();
  ...
  let concurrent_token = runtime
    .create_verification_token(
      TEST_VERIFICATION_TOKEN_TYPE,
      Some("concurrent@sisonotes.test".to_string()),
      30_000,
    )
    .await
    .unwrap();
  ...
  let expired_token = runtime
    .create_verification_token(TEST_VERIFICATION_TOKEN_TYPE, Some("expired@sisonotes.test".to_string()), 1)
    .await
    .unwrap();
  tokio::time::sleep(Duration::from_millis(20)).await;
  assert!(
