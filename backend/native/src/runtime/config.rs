...
impl Default for InviteQuotaConfig {
  fn default() -> Self {
    Self {
      ...
      .map(str::to_string)
      .collect(),
      subject_hash_salt: "sisonotes-runtime-invite-quota-v1-local".to_string(),
...
impl BackendRuntimeConfig {
  pub(crate) fn from_config_files() -> RuntimeResult<Self> {
    ...
    let database_url = database_url_from_env()
      ...
      .unwrap_or_else(|| "postgresql://localhost:5432/sisonotes".to_string());
    Ok(Self {
      database_url,
...
mod tests {
  ...
  fn config_paths_are_limited_to_executable_dir_and_cwd() {
    ...
    assert!(paths.iter().all(|path| !path.to_string_lossy().contains(".sisonotes")));
    assert!(
      paths
        .iter()
  ...
  fn blank_database_urls_are_ignored() {
    ...
    assert_eq!(non_empty_string("   ".to_string()), None);
    assert_eq!(
      non_empty_string("postgresql://sisonotes:sisonotes@localhost:5432/sisonotes".to_string()),
      Some("postgresql://sisonotes:sisonotes@localhost:5432/sisonotes".to_string())
    );
  }
